...
export const earningActivities = pgTable("earning_activities", {
  id: uuid("id").defaultRandom().primaryKey(),
  userId: uuid("user_id").references(() => users.id, { onDelete: 'cascade' }).notNull(),
  activityType: varchar("activity_type", { length: 32 }).notNull(), // 'post', 'comment', 'referral', 'marketplace', 'daily_login', 'profile_complete'
  activityId: varchar("activity_id", { length: 128 }), // ID of the related activity (post ID, comment ID, etc.)
  tokensEarned: numeric("tokens_earned", { precision: 20, scale: 8 }).notNull().default("0"),
  baseReward: numeric("base_reward", { precision: 20, scale: 8 }).notNull().default("0"),
  multiplier: numeric("multiplier", { precision: 5, scale: 2 }).notNull().default("1.0"),
  qualityScore: numeric("quality_score", { precision: 5, scale: 2 }).default("1.0"), // Quality scoring for content-based rewards
  isPremiumBonus: boolean("is_premium_bonus").default(false),
  premiumBonusAmount: numeric("premium_bonus_amount", { precision: 20, scale: 8 }).default("0"),
  dailyLimitApplied: boolean("daily_limit_applied").default(false),
  metadata: text("metadata"), // JSON object with additional activity data
  createdAt: timestamp("created_at").defaultNow().notNull(),
  ...
  activityTypeIdx: index("idx_earning_activities_activity_type").on(t.activityType),
  createdAtIdx: index("idx_earning_activities_created_at").on(t.createdAt),
  statusIdx: index("idx_earning_activities_status").on(t.status),
  activityIdIdx: index("idx_earning_activities_activity_id").on(t.activityId),
}));
...
export const referrals = pgTable("referrals", {
  ...
  tier: integer("tier").default(1), // Multi-tier referral support (1st tier, 2nd tier, etc.)
  bonusPercentage: numeric("bonus_percentage", { precision: 5, scale: 2 }).default("10.0"),
  totalEarned: numeric("total_earned", { precision: 20, scale: 8 }).default("0"),
  status: varchar("status", { length: 20 }).default("active"), // 'active', 'inactive', 'expired'
  expiresAt: timestamp("expires_at"),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
}, (t) => ({
  referrerIdx: index("idx_referrals_referrer_id").on(t.referrerId),
  refereeIdx: index("idx_referrals_referee_id").on(t.refereeId),
  referralCodeIdx: index("idx_referrals_referral_code").on(t.referralCode),
  statusIdx: index("idx_referrals_status").on(t.status),
  uniqueReferrerReferee: unique("idx_referrals_unique_referrer_referee").on(t.referrerId, t.refereeId),
}));
...
export const referralRewards = pgTable("referral_rewards", {
  ...
  earningActivityId: uuid("earning_activity_id").references(() => earningActivities.id, { onDelete: 'cascade' }),
  rewardAmount: numeric("reward_amount", { precision: 20, scale: 8 }).notNull(),
  rewardType: varchar("reward_type", { length: 32 }).notNull(), // 'signup_bonus', 'activity_bonus', 'milestone_bonus'
  milestoneReached: varchar("milestone_reached", { length: 64 }), // Description of milestone if applicable
  createdAt: timestamp("created_at").defaultNow().notNull(),
}, (t) => ({
  referralIdx: index("idx_referral_rewards_referral_id").on(t.referralId),
  earningActivityIdx: index("idx_referral_rewards_earning_activity_id").on(t.earningActivityId),
  createdAtIdx: index("idx_referral_rewards_created_at").on(t.createdAt),
}));

// Daily earning limits and tracking
export const dailyEarningLimits = pgTable("daily_earning_limits", {
  id: uuid("id").defaultRandom().primaryKey(),
  userId: uuid("user_id").references(() => users.id, { onDelete: 'cascade' }).notNull(),
  date: timestamp("date").notNull(),
  activityType: varchar("activity_type", { length: 32 }).notNull(),
  tokensEarnedToday: numeric("tokens_earned_today", { precision: 20, scale: 8 }).default("0"),
  dailyLimit: numeric("daily_limit", { precision: 20, scale: 8 }).notNull(),
  limitReached: boolean("limit_reached").default(false),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
}, (t) => ({
  userIdx: index("idx_daily_earning_limits_user_id").on(t.userId),
  dateIdx: index("idx_daily_earning_limits_date").on(t.date),
  activityTypeIdx: index("idx_daily_earning_limits_activity_type").on(t.activityType),
  uniqueUserDateActivity: unique("idx_daily_earning_limits_unique").on(t.userId, t.date, t.activityType),
}));
...
export const marketplaceRewards = pgTable("marketplace_rewards", {
  ...
  buyerReward: numeric("buyer_reward", { precision: 20, scale: 8 }).default("0"),
  sellerReward: numeric("seller_reward", { precision: 20, scale: 8 }).default("0"),
  rewardTier: varchar("reward_tier", { length: 32 }), // Volume-based bonus tier
  bonusMultiplier: numeric("bonus_multiplier", { precision: 5, scale: 2 }).default("1.0"),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  processedAt: timestamp("processed_at"),
}, (t) => ({
  orderIdx: index("idx_marketplace_rewards_order_id").on(t.orderId),
  buyerIdx: index("idx_marketplace_rewards_buyer_id").on(t.buyerId),
  sellerIdx: index("idx_marketplace_rewards_seller_id").on(t.sellerId),
  createdAtIdx: index("idx_marketplace_rewards_created_at").on(t.createdAt),
}));
...
export const earningChallenges = pgTable("earning_challenges", {
  ...
  name: varchar("name", { length: 255 }).notNull(),
  description: text("description"),
  challengeType: varchar("challenge_type", { length: 32 }).notNull(), // 'daily', 'weekly', 'monthly', 'milestone'
  activityType: varchar("activity_type", { length: 32 }), // Specific activity type or null for general challenges
  targetValue: numeric("target_value", { precision: 20, scale: 8 }).notNull(), // Target amount/count to achieve
  rewardAmount: numeric("reward_amount", { precision: 20, scale: 8 }).notNull(),
  bonusMultiplier: numeric("bonus_multiplier", { precision: 5, scale: 2 }).default("1.0"),
  startDate: timestamp("start_date"),
  endDate: timestamp("end_date"),
  isActive: boolean("is_active").default(true),
  maxParticipants: integer("max_participants"), // Limit number of participants
  currentParticipants: integer("current_participants").default(0),
  metadata: text("metadata"), // JSON object with additional challenge data
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
}, (t) => ({
  challengeTypeIdx: index("idx_earning_challenges_challenge_type").on(t.challengeType),
  activityTypeIdx: index("idx_earning_challenges_activity_type").on(t.activityType),
  isActiveIdx: index("idx_earning_challenges_is_active").on(t.isActive),
  startDateIdx: index("idx_earning_challenges_start_date").on(t.startDate),
  endDateIdx: index("idx_earning_challenges_end_date").on(t.endDate),
}));
...
export const userChallengeProgress = pgTable("user_challenge_progress", {
  ...
  userId: uuid("user_id").references(() => users.id, { onDelete: 'cascade' }).notNull(),
  currentProgress: numeric("current_progress", { precision: 20, scale: 8 }).default("0"),
  ...
  challengeIdx: index("idx_user_challenge_progress_challenge_id").on(t.challengeId),
  userIdx: index("idx_user_challenge_progress_user_id").on(t.userId),
  isCompletedIdx: index("idx_user_challenge_progress_is_completed").on(t.isCompleted),
  uniqueChallengeUser: unique("idx_user_challenge_progress_unique").on(t.challengeId, t.userId),
}));
...
export const activityFeed = pgTable("activity_feed", {
  ...
  title: varchar("title", { length: 255 }).notNull(),
  description: text("description"),
  tokensEarned: numeric("tokens_earned", { precision: 20, scale: 8 }).default("0"),
  icon: varchar("icon", { length: 64 }), // Icon identifier for UI
  actionUrl: varchar("action_url", { length: 500 }), // URL to navigate to for more details
  isRead: boolean("is_read").default(false),
  metadata: text("metadata"), // JSON object with additional data
  createdAt: timestamp("created_at").defaultNow().notNull(),
}, (t) => ({
  userIdx: index("idx_activity_feed_user_id").on(t.userId),
  activityTypeIdx: index("idx_activity_feed_activity_type").on(t.activityType),
  createdAtIdx: index("idx_activity_feed_created_at").on(t.createdAt),
  isReadIdx: index("idx_activity_feed_is_read").on(t.isRead),
}));

// Earning configuration and rules
export const earningConfig = pgTable("earning_config", {
  id: uuid("id").defaultRandom().primaryKey(),
  activityType: varchar("activity_type", { length: 32 }).notNull().unique(),
  baseReward: numeric("base_reward", { precision: 20, scale: 8 }).notNull().default("0"),
  dailyLimit: numeric("daily_limit", { precision: 20, scale: 8 }).default("100"), // Default daily limit
  qualityMultiplierEnabled: boolean("quality_multiplier_enabled").default(false),
  premiumBonusPercentage: numeric("premium_bonus_percentage", { precision: 5, scale: 2 }).default("25.0"),
  cooldownPeriod: integer("cooldown_period").default(0), // Cooldown in seconds between rewards
  minQualityScore: numeric("min_quality_score", { precision: 5, scale: 2 }).default("0.5"),
  maxQualityScore: numeric("max_quality_score", { precision: 5, scale: 2 }).default("2.0"),
  isActive: boolean("is_active").default(true),
  metadata: text("metadata"), // JSON object with additional configuration
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
}, (t) => ({
  activityTypeIdx: index("idx_earning_config_activity_type").on(t.activityType),
  isActiveIdx: index("idx_earning_config_is_active").on(t.isActive),
}));

// User earning statistics for analytics
export const userEarningStats = pgTable("user_earning_stats", {
  id: uuid("id").defaultRandom().primaryKey(),
  userId: uuid("user_id").references(() => users.id, { onDelete: 'cascade' }).notNull(),
  totalTokensEarned: numeric("total_tokens_earned", { precision: 20, scale: 8 }).default("0"),
  tokensEarnedToday: numeric("tokens_earned_today", { precision: 20, scale: 8 }).default("0"),
  tokensEarnedThisWeek: numeric("tokens_earned_this_week", { precision: 20, scale: 8 }).default("0"),
  tokensEarnedThisMonth: numeric("tokens_earned_this_month", { precision: 20, scale: 8 }).default("0"),
  ...
  currentStreak: integer("current_streak").default(0), // Daily activity streak
  longestStreak: integer("longest_streak").default(0),
  lastActivityDate: timestamp("last_activity_date"),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
}, (t) => ({
  userIdx: index("idx_user_earning_stats_user_id").on(t.userId),
  totalTokensEarnedIdx: index("idx_user_earning_stats_total_tokens_earned").on(t.totalTokensEarned),
  currentStreakIdx: index("idx_user_earning_stats_current_streak").on(t.currentStreak),
  uniqueUser: unique("idx_user_earning_stats_unique_user").on(t.userId),
}));

// Abuse prevention and rate limiting
export const earningAbusePrevention = pgTable("earning_abuse_prevention", {
  id: uuid("id").defaultRandom().primaryKey(),
  userId: uuid("user_id").references(() => users.id, { onDelete: 'cascade' }).notNull(),
  activityType: varchar("activity_type", { length: 32 }).notNull(),
  ...
  userIdx: index("idx_earning_abuse_prevention_user_id").on(t.userId),
  activityTypeIdx: index("idx_earning_abuse_prevention_activity_type").on(t.activityType),
...
export const stakingTiers = pgTable("staking_tiers", {
  id: serial("id").primaryKey(),
  name: varchar("name", { length: 100 }).notNull(),
  lockPeriod: integer("lock_period").notNull().default(0), // in seconds, 0 for flexible
  baseAprRate: integer("base_apr_rate").notNull(), // basis points (e.g., 500 = 5%)
  premiumBonusRate: integer("premium_bonus_rate").notNull().default(0), // additional APR for premium members
  minStakeAmount: varchar("min_stake_amount", { length: 50 }).notNull(), // in wei as string
  maxStakeAmount: varchar("max_stake_amount", { length: 50 }), // in wei as string, NULL for unlimited
  isActive: boolean("is_active").notNull().default(true),
  allowsAutoCompound: boolean("allows_auto_compound").notNull().default(false),
  earlyWithdrawalPenalty: integer("early_withdrawal_penalty").notNull().default(0), // basis points
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});
...
export const stakingPositions = pgTable("staking_positions", {
  ...
  amount: varchar("amount", { length: 50 }).notNull(), // in wei as string
  startTime: timestamp("start_time").notNull(),
  lockPeriod: integer("lock_period").notNull(), // in seconds
  aprRate: integer("apr_rate").notNull(), // effective APR including premium bonus
  lastRewardClaim: timestamp("last_reward_claim").notNull(),
  accumulatedRewards: varchar("accumulated_rewards", { length: 50 }).notNull().default('0'),
  isActive: boolean("is_active").notNull().default(true),
  isAutoCompound: boolean("is_auto_compound").notNull().default(false),
  isFixedTerm: boolean("is_fixed_term").notNull().default(false),
  tierId: integer("tier_id").notNull().references(() => stakingTiers.id),
  contractAddress: varchar("contract_address", { length: 42 }).notNull(),
  transactionHash: varchar("transaction_hash", { length: 66 }).notNull(),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
}, (table) => ({
  userIdIdx: index("idx_staking_positions_user_id").on(table.userId),
  activeIdx: index("idx_staking_positions_active").on(table.isActive),
  tierIdIdx: index("idx_staking_positions_tier_id").on(table.tierId),
  walletIdx: index("idx_staking_positions_wallet").on(table.walletAddress),
}));

// User staking summary information
export const userStakingInfo = pgTable("user_staking_info", {
  userId: varchar("user_id", { length: 100 }).primaryKey(),
  totalStaked: varchar("total_staked", { length: 50 }).notNull().default('0'),
  totalRewards: varchar("total_rewards", { length: 50 }).notNull().default('0'),
  activePositions: integer("active_positions").notNull().default(0),
  isPremiumMember: boolean("is_premium_member").notNull().default(false),
  premiumMemberSince: timestamp("premium_member_since"),
  lastActivityTime: timestamp("last_activity_time").defaultNow(),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
}, (table) => ({
  premiumIdx: index("idx_user_staking_info_premium").on(table.isPremiumMember),
}));
...
export const stakingEvents = pgTable("staking_events", {
  ...
  eventType: varchar("event_type", { length: 50 }).notNull(), // 'stake', 'unstake', 'claim_rewards', 'auto_compound', 'partial_unstake'
  amount: varchar("amount", { length: 50 }).notNull(),
  penaltyAmount: varchar("penalty_amount", { length: 50 }).default('0'),
  transactionHash: varchar("transaction_hash", { length: 66 }).notNull(),
  ...
  userIdIdx: index("idx_staking_events_user_id").on(table.userId),
  typeIdx: index("idx_staking_events_type").on(table.eventType),
  createdAtIdx: index("idx_staking_events_created_at").on(table.createdAt),
}));

// Auto-compound queue for batch processing
export const autoCompoundQueue = pgTable("auto_compound_queue", {
  id: serial("id").primaryKey(),
  positionId: varchar("position_id", { length: 100 }).notNull().references(() => stakingPositions.id),
  userId: varchar("user_id", { length: 100 }).notNull(),
