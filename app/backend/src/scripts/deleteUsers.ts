...
async function deleteUsers() {
  ...
  if (userIds.length === 0) {
    ...
    console.log('\nUsage: npx ts-node src/scripts/deleteUsers.ts <userId1> <userId2> ...');
    console.log('\nExample:');
    console.log('  npx ts-node src/scripts/deleteUsers.ts 508c0fb3-a07e-416a-a1ac-9169aacf6422 4419945e-5c96-419b-9f85-6642ec747084');
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(`Found ${existingUsers.length} user(s) to delete:`);
    existingUsers.forEach(user => {
      console.log(`  - ${user.id} (${user.walletAddress || 'no wallet'}, role: ${user.role || 'user'})`);
    });
    ...
    console.log('  ✓ Deleted from follows table');

    // Delete other potential references
    try {
      await sql`DELETE FROM posts WHERE author_id = ANY(${foundUserIds})`;
      console.log('  ✓ Deleted from posts table');
    } catch (e) {
      console.log('  ℹ️  No posts to delete or already cascaded');
    }

    try {
      await sql`DELETE FROM comments WHERE author_id = ANY(${foundUserIds})`;
      console.log('  ✓ Deleted from comments table');
    } catch (e) {
      console.log('  ℹ️  No comments to delete or already cascaded');
    }

    try {
      await sql`DELETE FROM reactions WHERE user_id = ANY(${foundUserIds})`;
      console.log('  ✓ Deleted from reactions table');
    } catch (e) {
      console.log('  ℹ️  No reactions to delete or already cascaded');
    }

    try {
      await sql`DELETE FROM messages WHERE sender_id = ANY(${foundUserIds}) OR recipient_id = ANY(${foundUserIds})`;
      console.log('  ✓ Deleted from messages table');
    } catch (e) {
      console.log('  ℹ️  No messages to delete or already cascaded');
    }

    try {
      await sql`DELETE FROM bids WHERE bidder_id = ANY(${foundUserIds})`;
      console.log('  ✓ Deleted from bids table');
    } catch (e) {
      console.log('  ℹ️  No bids to delete or already cascaded');
    }

    try {
      await sql`DELETE FROM seller_profiles WHERE user_id = ANY(${foundUserIds})`;
      console.log('  ✓ Deleted from seller_profiles table');
    } catch (e) {
      console.log('  ℹ️  No seller profiles to delete or already cascaded');
    }

    try {
      await sql`DELETE FROM marketplace_listings WHERE seller_id = ANY(${foundUserIds})`;
      console.log('  ✓ Deleted from marketplace_listings table');
    } catch (e) {
      console.log('  ℹ️  No listings to delete or already cascaded');
    }

    try {
      await sql`DELETE FROM orders WHERE buyer_id = ANY(${foundUserIds}) OR seller_id = ANY(${foundUserIds})`;
      console.log('  ✓ Deleted from orders table');
    } catch (e) {
      console.log('  ℹ️  No orders to delete or already cascaded');
    }

    try {
      await sql`DELETE FROM listings WHERE seller_id = ANY(${foundUserIds})`;
      console.log('  ✓ Deleted from listings table');
    } catch (e) {
      console.log('  ℹ️  No listings to delete or already cascaded');
    }
    ...
    try {
      ...
      console.log('  ✓ Deleted from reviews table');
    } catch (e) {
      console.log('  ℹ️  No reviews to delete or already cascaded');
    }

    try {
      await sql`DELETE FROM disputes WHERE complainant_id = ANY(${foundUserIds}) OR respondent_id = ANY(${foundUserIds})`;
      console.log('  ✓ Deleted from disputes table');
    } catch (e) {
      console.log('  ℹ️  No disputes to delete or already cascaded');
    }
