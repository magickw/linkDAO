 ...
 */
export const USDC_ADDRESSES: TokenConfig = {
    symbol: 'USDC',
    name: 'USD Coin',
    decimals: 6,
    addresses: {
 ...
 */
export const USDT_ADDRESSES: TokenConfig = {
    symbol: 'USDT',
    name: 'Tether USD',
    decimals: 6,
    addresses: {
 ...
 */
export function getTokenByAddress(address: string, chainId: number): TokenConfig | null {
    const normalizedAddress = address.toLowerCase();

    if (normalizedAddress === USDC_ADDRESSES.addresses[chainId]?.toLowerCase()) {
        return USDC_ADDRESSES;
    }

    if (normalizedAddress === USDT_ADDRESSES.addresses[chainId]?.toLowerCase()) {
