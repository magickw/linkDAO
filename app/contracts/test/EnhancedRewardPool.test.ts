...
describe("EnhancedRewardPool", function () {
  ...
  beforeEach(async function () {
    ...
    await ldaoToken.connect(funder).approve(await rewardPool.getAddress(), ethers.parseEther("10000"));
    await ldaoToken.connect(owner).approve(await rewardPool.getAddress(), ethers.parseEther("10000"));
  });

  describe("Deployment", function () {
    it("Should set the correct token and contract addresses", async function () {
      expect(await rewardPool.ldaoToken()).to.equal(await ldaoToken.getAddress());
      expect(await rewardPool.governance()).to.equal(await governance.getAddress());
      expect(await rewardPool.reputationSystem()).to.equal(await reputationSystem.getAddress());
    });
