...
async function main() {
  ...
  try {
    await paymentRouter.setTokenSupported(tokenAddress, true);
  ...
  const addresses = {
    ...
    FOLLOW_MODULE_ADDRESS: followModuleAddress,
    PAYMENT_ROUTER_ADDRESS: paymentRouterAddress,
  ...
  console.log(`FOLLOW_MODULE_ADDRESS=${followModuleAddress}`);
