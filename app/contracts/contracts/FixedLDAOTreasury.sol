...
/**
 * @title FixedLDAOTreasury
 * @notice Enhanced Treasury contract for LDAO token sales with multi-sig controls and circuit breakers
 * @dev Fixed version that uses Chainlink price oracles instead of hardcoded ETH price
 */
contract FixedLDAOTreasury is Ownable, ReentrancyGuard, Pausable {
    ...
    function purchaseWithETH(uint256 ldaoAmount) external payable nonReentrant whenNotPaused {
        ...
        uint256 ethPriceInUSD = _getETHPriceFromOracle();
        uint256 requiredETH;
        unchecked {
    ...
    /**
     * @notice Update Chainlink ETH/USD price feed address
