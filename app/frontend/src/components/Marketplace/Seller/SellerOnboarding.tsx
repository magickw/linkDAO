...
function SellerOnboardingComponent({ onComplete }: SellerOnboardingProps) {
  ...
  const [stepData, setStepData] = useState<Record<string, any>>(() => {
    if (typeof window !== 'undefined' && address) {
      try {
        const saved = localStorage.getItem(`onboarding_data_${address}`);
  ...
  React.useEffect(() => {
    if (address && Object.keys(stepData).length > 0) {
  ...
  React.useEffect(() => {
    if (isCompleted && address) {
      localStorage.removeItem(`onboarding_data_${address}`);
    }
  }, [isCompleted, address]);
        ...
        <GlassPanel className="max-w-md w-full text-center">
          <div className="mb-6">
            ...
            <p className="text-gray-300 mb-6">
              Your seller account is now set up and ready to go. Redirecting you to your dashboard...
            </p>
          </div>
  ...
  if (!currentStepData || !Array.isArray(steps) || steps.length === 0) {
    return (
      <div className="text-center py-8">
        <GlassPanel className="max-w-md mx-auto">
          <div className="text-yellow-400 mb-4">
            ...
            <p className="text-lg font-semibold">No Onboarding Steps Available</p>
            <p className="text-sm text-gray-300 mt-2">Please try refreshing the page</p>
          </div>
          <Button onClick={() => window.location.reload()} variant="primary">
            Refresh Page
          </Button>
