      ...
      <div className="flex">
        <div className="flex-shrink-0 mr-4">
          <img 
            ...
            src={profile.avatarCid || 'https://via.placeholder.com/40'} 
            alt={profile.handle} 
          />
        </div>
        <div className="flex-1">
          <div className="flex items-center">
            <Link href={`/profile/${post.author}`} className="text-sm font-medium text-gray-900 hover:text-primary-600">
              {profile.handle}
            </Link>
            {profile.ens && (
              <span className="ml-2 text-sm text-gray-500">({profile.ens})</span>
            )}
