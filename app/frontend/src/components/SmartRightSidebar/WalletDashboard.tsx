...
const WalletDashboard = React.memo(function WalletDashboard({ 
  ...
  // Debug indicator: determine if we're showing real or mock data
  ...
  const lastUpdated = displayWalletData ? new Date() : null;

  // Animate portfolio value changes - but throttle to prevent excessive animations
  const [lastPortfolioValue, setLastPortfolioValue] = useState(displayWalletData?.portfolioValue || 0);
  
  useEffect(() => {
    if (!displayWalletData) return;
  ...
  const isPositive = displayWalletData?.portfolioChange ? displayWalletData.portfolioChange >= 0 : true;
      ...
      <div className="px-4 pb-4">
        <div className="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
          <div className="flex items-center">
            <div className={`w-2 h-2 rounded-full mr-2 ${isUsingRealData ? 'bg-green-500 animate-pulse' : 'bg-gray-400'}`} />
            {isUsingRealData ? 'Live blockchain data' : 'Wallet data'}
          </div>
