...
import { Users, CheckSquare, Square, Search, Loader2 } from 'lucide-react';
...
export default function SharePostModal({
  ...
  useEffect(() => {
    if (isOpen && address && selectedOption === 'community') {
      const fetchCommunities = async () => {
        try {
          setLoadingCommunities(true);
          const response = await CommunityService.getMyCommunities(1, 20);
  ...
  useEffect(() => {
    if (selectedOption !== 'community' || !searchQuery.trim()) {
      setSearchResults([]);
      return;
    }
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        // Using searchCommunitiesAdvanced for better results if available, otherwise searchCommunities
        const results = await CommunityService.searchCommunities(searchQuery.trim(), 10);
  ...
  const shareOptions: ShareOption[] = [
    ...
    if (option.id === 'community') {
      ...
      // RESTRICTED: Only the original author can share to a community
    ...
    >
      ...
      >
        ...
        {selectedOption === 'community' && (
          <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex flex-col min-h-0 flex-1">
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              Select a community
            </label>
            ...
            <div className="relative mb-3">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none transition-all"
              />
            ...
            {/* Communities List */}
            <div className="flex-1 overflow-y-auto max-h-60 border border-gray-200 dark:border-gray-700 rounded-lg">
              {loadingCommunities && !searchQuery ? (
                <div className="p-4 text-center text-gray-500 dark:text-gray-400">
                  <Loader2 className="w-6 h-6 animate-spin mx-auto mb-2" />
                  <p className="text-sm">Loading your communities...</p>
                </div>
              ) : (
                <>
                  {/* Results Header */}
                  <div className="px-3 py-2 bg-gray-50 dark:bg-gray-800/50 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider border-b border-gray-200 dark:border-gray-700">
                    {searchQuery ? 'Search Results' : 'Your Communities'}
                  </div>

                  {/* List Content */}
                  {(searchQuery ? searchResults : userCommunities).length === 0 ? (
                    <div className="p-8 text-center">
                      <Users className="w-8 h-8 text-gray-300 dark:text-gray-600 mx-auto mb-2" />
                      <p className="text-sm text-gray-500 dark:text-gray-400">
                        {searchQuery ? 'No matching communities found' : 'You haven\'t joined any communities yet'}
                      </p>
                    </div>
                  ) : (
                    <div className="divide-y divide-gray-100 dark:divide-gray-700">
                      {(searchQuery ? searchResults : userCommunities).map((community) => {
                        ...
                        return (
                          <button
                            ...
                            className={`w-full flex items-center p-3 transition-colors text-left hover:bg-gray-50 dark:hover:bg-gray-700/50 ${
                          ...
                          >
                            <div className="w-10 h-10 rounded-full overflow-hidden flex-shrink-0 mr-3">
                              {community.avatar ? (
                                ...
                                <div className="w-full h-full bg-gradient-to-br from-primary-400 to-secondary-500 flex items-center justify-center text-white text-xs font-bold">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center">
                                ...
                                {isJoined && (
                                  <span className="ml-2 px-1.5 py-0.5 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 text-[10px] font-bold rounded uppercase">
                                    Joined
                                  </span>
                                )}
                              </div>
                            ...
                            {isSelected && (
                              <CheckSquare className="w-5 h-5 text-primary-600 dark:text-primary-400 ml-2" />
                            )}
                          </button>
                        );
                      })}
            ...
            {/* Selected Community Preview (Optional, now integrated into list but keeping for clarity if list is long) */}
            {selectedCommunityId && !((searchQuery ? searchResults : userCommunities).some(c => c.id === selectedCommunityId)) && (
              <div className="mt-3 p-3 bg-primary-50 dark:bg-primary-900/20 rounded-lg border border-primary-200 dark:border-primary-700">
                <p className="text-xs font-medium text-primary-700 dark:text-primary-300 mb-2">Currently selected:</p>
                <div className="flex items-center space-x-3">
                  {/* We might need to fetch the selected community if it's not in the current list, 
                      but for now we assume it's in the list or was just selected */}
