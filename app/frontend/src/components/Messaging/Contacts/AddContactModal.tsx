...
import { XMarkIcon, UserPlusIcon, CheckIcon, PlusIcon, MagnifyingGlassIcon, ExclamationTriangleIcon, CheckBadgeIcon } from '@heroicons/react/24/outline';
...
const AddContactModal: React.FC<AddContactModalProps> = ({ isOpen, onClose }) => {
  ...
  const [platformStatus, setPlatformStatus] = useState<'verified' | 'unverified' | 'not_found' | null>(null);
  ...
  const handleSearch = async () => {
    ...
    try {
      const profiles = await ProfileService.searchProfiles(searchQuery);

      if (profiles.length > 0) {
        const profile = profiles[0]; // Take the first match
        setPlatformStatus('verified');
        ...
        setFormData(prev => ({
          ...
          ensName: profile.ens || '',
          nickname: profile.displayName || profile.handle || '',
        }));
      } else {
        // Check if it's a valid address even if not on platform
        if (/^0x[a-fA-F0-9]{40}$/.test(searchQuery)) {
          setPlatformStatus('unverified');
          setFormData(prev => ({
            ...prev,
            walletAddress: searchQuery,
            nickname: '', // Let user fill
            ensName: ''
          }));
        } else {
          setPlatformStatus('not_found');
        }
      }
    } catch (err) {
      console.error(err);
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <motion.div
          ...
          className="relative w-full max-w-md mx-4 bg-gray-800 rounded-xl shadow-xl border border-gray-700 max-h-[90vh] overflow-y-auto"
        >
          {/* Header */}
          <div className="flex items-center justify-between p-6 border-b border-gray-700 sticky top-0 bg-gray-800 z-10">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-400">Search and add users</p>
              </div>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="space-y-2">
              ...
              {platformStatus === 'verified' && (
                <div className="flex items-center gap-2 p-3 bg-green-900/30 border border-green-700 rounded-lg text-green-400 text-sm">
                  <CheckBadgeIcon className="w-5 h-5" />
                  <span>User found on platform! Details auto-filled.</span>
                </div>
              )}
              {platformStatus === 'unverified' && (
                <div className="flex items-center gap-2 p-3 bg-yellow-900/30 border border-yellow-700 rounded-lg text-yellow-400 text-sm">
                  <ExclamationTriangleIcon className="w-5 h-5" />
                  <span>User not on platform. You can still add them manually.</span>
                </div>
              )}
              {platformStatus === 'not_found' && (
                <div className="flex items-center gap-2 p-3 bg-red-900/30 border border-red-700 rounded-lg text-red-400 text-sm">
                  <XMarkIcon className="w-5 h-5" />
                  <span>User not found. Please check the address or name.</span>
                </div>
              )}
            </div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {GROUP_ICONS.map(icon => (
                          <button
                            ...
                            className={`w-8 h-8 text-lg rounded border-2 transition-all ${selectedIcon === icon
                ...
                <div className="flex flex-wrap gap-2">
                  {CONTACT_TAGS.map(tag => (
                    <button
                      ...
                      className={`px-3 py-1 text-xs rounded-full border transition-colors ${selectedTags.has(tag)
