...
export const MessageBubble: React.FC<MessageBubbleProps> = ({
  ...
  const renderMessageContent = () => {
    switch (message.contentType) {
      case 'image':
        // Check if there's an attachment with the image URL
        const imageAttachment = message.attachments?.find(att => att.type === 'image');
        const imageUrl = imageAttachment?.url || (message.content.startsWith('http') ? message.content : null);
            ...
            <img 
              ...
              onError={(e) => {
                ...
                e.currentTarget.parentElement?.insertAdjacentHTML('afterend', `
                  <div class="flex items-center space-x-2 p-4 bg-gray-100 dark:bg-gray-700 rounded-lg">
                    <svg class="w-8 h-8 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                    </svg>
                    <div>
                      <p class="text-sm font-medium text-gray-900 dark:text-white">Image</p>
