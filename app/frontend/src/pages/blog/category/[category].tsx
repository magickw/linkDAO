...
export async function getStaticProps({ params }: { params: { category: string } }) {
  ...
  const categoryTags = categoryMap[category] || [category];
...
const BlogCategoryPage: React.FC<{ 
  ...
  categoryName: string; 
  posts: BlogPost[] 
}> = ({ category, categoryName, posts }) => {
  return (
    <>
      <Head>
        <title>{categoryName} | LinkDAO Blog</title>
        <meta name="description" content={`All blog posts in the ${categoryName} category`} />
      </Head>
        ...
        <div className="bg-gradient-to-r from-blue-600 to-indigo-700 text-white py-16">
          <div className="container mx-auto px-4">
            ...
            <p className="text-xl md:text-2xl max-w-3xl">
              All blog posts in the {categoryName.toLowerCase()} category
            </p>
          </div>
        </div>
