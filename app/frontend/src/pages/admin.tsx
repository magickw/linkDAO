...
export default function AdminDashboard() {
  const [activeTab, setActiveTab] = useState<'overview' | 'moderation' | 'policies' | 'thresholds' | 'vendors' | 'audit'>('overview');
  ...
  const tabs = [
    ...
    { id: 'moderation', name: 'Moderation Dashboard', icon: '🛡️' },
    { id: 'policies', name: 'Policy Configuration', icon: '📋' },
    { id: 'thresholds', name: 'Threshold Tuning', icon: '⚖️' },
    ...
    <Layout title="Admin Dashboard - LinkDAO">
      <div className="px-4 py-6 sm:px-0">
        <div className="max-w-7xl mx-auto">
          ...
          {activeTab === 'moderation' && <SystemStatusDashboard />}
          {activeTab === 'policies' && <PolicyConfigurationPanel />}
          {activeTab === 'thresholds' && <ThresholdTuningPanel />}
          {activeTab === 'vendors' && <VendorConfigurationPanel />}
          {activeTab === 'audit' && <AuditLogAnalysis />}
        </div>
      </div>
    </Layout>
  );
