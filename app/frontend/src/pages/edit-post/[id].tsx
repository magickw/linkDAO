...
const EditPostPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!address) {
      addToast('Please connect your wallet to edit a post', 'error');
      return;
    }

    if (!id) {
      addToast('Post ID is missing', 'error');
      return;
    }
    ...
    try {
      ...
      // Dispatch event to notify other components that a post was updated
      ...
      addToast(error instanceof Error ? error.message : 'Failed to update post. Please try again.', 'error');
    } finally {
  ...
  if (isLoading) {
    return (
      <ErrorBoundary>
        <Layout title="Edit Post - LinkDAO">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div className="flex items-center text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-200 mb-4">
