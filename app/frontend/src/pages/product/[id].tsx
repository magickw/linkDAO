            ...
            const transformedProduct = {
              ...
              media: (Array.isArray(productData.images) ? productData.images : []).length > 0 ? 
                productData.images.map((url: string, index: number) => ({
                  type: 'image' as const,
                  url: url || `https://placehold.co/600x400/667eea/ffffff?text=${encodeURIComponent(productData.title || `Product ${index + 1}`)}`,
                  thumbnail: url || `https://placehold.co/150x150/667eea/ffffff?text=${encodeURIComponent(productData.title || `Product ${index + 1}`)}`,
                  alt: productData.title ? `${productData.title} - Image ${index + 1}` : `Product Image ${index + 1}`
                })) : [
                  {
                    type: 'image' as const,
                    url: `https://placehold.co/600x400/667eea/ffffff?text=${encodeURIComponent(productData.title || 'Product')}`,
                    thumbnail: `https://placehold.co/150x150/667eea/ffffff?text=${encodeURIComponent(productData.title || 'Product')}`,
