 ...
 *
 * Centralized state management for the checkout process
 * Replaces multiple useState calls with a single useReducer
 */

import { ShippingAddress } from '@/hooks/useCheckoutFlow';
import { PrioritizedPaymentMethod, PrioritizationResult } from '@/types/paymentPrioritization';
import { CheckoutRecommendation } from '@/services/unifiedCheckoutService';
...
export function canProceedFromStep(state: CheckoutState): boolean {
  switch (state.currentStep) {
    ...
    case 'payment-details':
      ...
      if (state.selectedPaymentMethod.type === 'crypto') {
        return state.selectedNetwork !== null && state.selectedToken !== null;
      }

      if (state.selectedPaymentMethod.type === 'fiat') {
