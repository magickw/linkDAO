  ...
  async purchaseWithCrypto(fromToken: 'ETH' | 'USDC', ldaoAmount: string): Promise<PurchaseResult> {
    try {
      ...
      if (fromToken === 'ETH') {
        ...
        const quote = await this.getQuote(ldaoAmount);
        const usdcAmount = ethers.utils.parseUnits(quote.usdcAmount, 6);
        
        // Approve USDC spending
        const approveTx = await usdcContract.approve(this.treasuryContract.address, usdcAmount);
  ...
  async getQuote(ldaoAmount: string): Promise<PurchaseQuote> {
    try {
      ...
      return {
        ...
        discount: discount.toNumber() / 100, // Convert from basis points
        fees: {
