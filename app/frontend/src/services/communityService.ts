...
export class CommunityService {
  ...
  static async getCommunityBySlug(slug: string): Promise<Community | null> {
    ...
    try {
      // Special case for 'linkdao' - create a default community if it doesn't exist
      if (slug === 'linkdao') {
        try {
          const response = await fetchWithRetry(
          ...
          // If we found the community, return it normally
          if (response.ok) {
            const data = await response.json();
            ...
            // If the community doesn't exist (404), return a default LinkDAO community
            if (response.status === 404) {
              console.warn(`LinkDAO community not found, returning default community`);
              const defaultLinkDAO: Community = {
                id: 'linkdao-default',
                name: 'linkdao',
                slug: 'linkdao',
                displayName: 'LinkDAO Community',
                description: 'The main community for LinkDAO discussions, governance, and announcements',
                memberCount: 1000,
                postCount: 500,
                avatar: '🏛️',
                banner: 'https://placehold.co/800x200/1e40af/white?text=LinkDAO+Community',
                category: 'dao',
              ...
              // Cache the default community
              await this.offlineCacheService.cacheCommunity(defaultLinkDAO);
              return defaultLinkDAO;
            }
          ...
          // On any error for 'linkdao', return the default community
          console.warn('Error fetching LinkDAO community, returning default:', error);
          const defaultLinkDAO: Community = {
            id: 'linkdao-default',
            name: 'linkdao',
            slug: 'linkdao',
      ...
      // For non-linkdao communities, use normal flow
      const response = await fetchWithRetry(
        `${BACKEND_API_BASE_URL}${API_ENDPOINTS.COMMUNITIES}/${slug}`,
        {
          method: 'GET',
      ...
      // For 'linkdao' specifically, always return a default community on error
      if (slug === 'linkdao') {
        console.warn('Error fetching LinkDAO community, returning default:', error);
        const defaultLinkDAO: Community = {
          id: 'linkdao-default',
      ...
      // Try to get from cache on network errors for other communities
      if (error instanceof Error && (error.name === 'AbortError' || error.name === 'TypeError')) {
        const cached = await this.offlineCacheService.getCachedCommunity(slug);
        if (cached) {
          console.warn('Network error, returning cached community data');
